...
export function useFolders() {
  ...
  const dispatch = useCallback((action: FolderAction) => {
      ...
      try {
        switch (action.type) {
          case 'CREATE_FOLDER':
            ...
            // 获取当前用户ID，异步处理
            const createFolderWithUserId = async () => {
            ...
            const newFolder: Folder = {
              ...
              userId: 'temporary', // 临时值，将在后续更新
              createdAt: new Date(),
              updatedAt: new Date()
            }

            // 异步设置正确的用户ID
            createFolderWithUserId().then(userId => {
              setFolders(prevFolders =>
    ...
    const triggerDecoupledSync = () => {
      try {
        // TODO: Re-implement sync trigger after T015 cleanup
        if (action.type === 'CREATE_FOLDER') {
          console.log('☁️ 触发文件夹创建同步...')
          // triggerFolderSync('create', action.payload) // Removed in T015 cleanup
        } else if (action.type === 'UPDATE_FOLDER') {
          console.log('☁️ 触发文件夹更新同步...')
          // triggerFolderSync('update', { id: action.payload.id, updates: action.payload.updates }) // Removed in T015 cleanup
        } else if (action.type === 'DELETE_FOLDER') {
          console.log('☁️ 触发文件夹删除同步...')
          // triggerFolderSync('delete', action.payload) // Removed in T015 cleanup
        } else if (action.type === 'TOGGLE_FOLDER') {
          console.log('☁️ 触发文件夹展开状态同步...')
          // triggerFolderSync('toggle', { id: action.payload, isExpanded: !getFolderById(action.payload)?.isExpanded }) // Removed in T015 cleanup
        }
      } catch (error) {
    ...
    try {
      ...
      if (folders.length > 0) {
        ...
        if (foldersWithoutSync.length > 0) {
          ...
          // 仅在确实需要时才自动修复同步字段
          const repairFolderUserIds = async () => {
            const currentUserId = await getCurrentUserId()
            return folders.map(folder => ({
              ...folder,
              syncVersion: folder.syncVersion || 1,
              pendingSync: folder.pendingSync || false,
              userId: folder.userId || currentUserId || 'anonymous'
            }))
          }

          const repairedFolders = await repairFolderUserIds()
          // 只在确实有变化时才更新状态
            ...
            if (dbFolders.length > 0) {
              ...
              foldersToLoad = dbFolders.map(folder => {
                ...
                return {
                  ...
                  syncVersion: folder.syncVersion || 1,
                  pendingSync: folder.pendingSync || false,
                  userId: folder.userId || 'anonymous',
  ...
  useEffect(() => {
    ...
    const saveFolders = async () => {
      try {
        ...
        const normalizedFolders = await normalizeFolders()
        ...
        try {
          // 规范化数据后重新保存
          const normalizeFoldersForRetry = async () => {
            const currentUserId = await getCurrentUserId()
            return folders.map(folder => {
              const children = folders.filter(f => f.parentId === folder.id)
              const hasChildren = children.length > 0
          ...
          const normalizedFolders = await normalizeFoldersForRetry()
