...
export function useCards() {
  ...
  const dispatch = useCallback(async (action: CardAction): Promise<void> => {
    ...
    setCards(prevCards => {
      switch (action.type) {
        case 'CREATE_CARD': {
          ...
          // 立即发布卡片创建事件（非阻塞）
          publishCardCreated(newCard).catch(error => {
        ...
        case 'UPDATE_CARD': {
          const existingCard = prevCards.find(card => card.id === action.payload.id)
          ...
          const changes = Object.keys(action.payload.updates || {}).reduce((acc, key) => {
            acc[key] = {
              from: existingCard[key as keyof Card],
          ...
          // 立即发布卡片更新事件（非阻塞）
          publishCardUpdated(updatedCard, changes).catch(error => {
            console.warn('发布卡片更新事件失败:', error)
          })

          return prevCards.map(card =>
            card.id === action.payload.id ? updatedCard : card
          )
        }

        case 'DELETE_CARD': {
          const deletedCard = prevCards.find(card => card.id === action.payload)
          const updatedCards = prevCards.filter(card => card.id !== action.payload)

          // 立即发布卡片删除事件（非阻塞）
          publishCardDeleted(action.payload, deletedCard).catch(error => {
        ...
        case 'DUPLICATE_CARD': {
          ...
          // 立即发布卡片创建事件（非阻塞）
          publishCardCreated(duplicatedCard).catch(error => {
        ...
        case 'MOVE_TO_FOLDER': {
          const updatedCards = prevCards.map(card => {
            if (card.id === action.payload.cardId) {
              ...
              // 立即发布卡片更新事件（非阻塞）
              publishCardUpdated(updatedCard, { folderId: action.payload.folderId }).catch(error => {
          ...
          // 发布批量更新事件（非阻塞）
          publishCardsBulkUpdated([action.payload.cardId], 'move-to-folder', updatedCards.filter(card => card.id === action.payload.cardId)).catch(error => {
            console.warn('发布批量移动事件失败:', error)
          })
    ...
    // 请求后台同步（非阻塞，不等待完成）
    requestSync('incremental', 'normal').catch(error => {
      console.warn('请求同步失败:', error)
    })
  }, [requestSync])
  ...
  useEffect(() => {
    ...
    const saveTimer = setTimeout(async () => {
      try {
        ...
        // 非阻塞保存：启动保存但不等待完成
        universalStorageAdapter.saveCards(cardsForSave).then(() => {
          console.log(`自动保存成功：${cardsForSave.length} 张卡片 (使用 ${universalStorageAdapter.getStorageMode()} 存储模式)`)
        }).catch(saveError => {
          console.error('保存卡片到持久化存储失败:', saveError)
          errorHandler.handleError(saveError, '保存卡片数据失败').catch(error => {
            console.warn('处理保存错误失败:', error)
          })

          // 尝试回退到DataConverterAdapter（非阻塞）
          try {
            DataConverterAdapter.saveToLocalStorage(cardsForSave)
            console.log('回退保存成功')
          } catch (fallbackError) {
            console.error('回退保存也失败:', fallbackError)
            errorHandler.handleError(fallbackError, '回退保存失败').catch(error => {
              console.warn('处理回退保存错误失败:', error)
            })
          }
        })
        ...
        // 请求后台同步（非阻塞）
        requestSync('incremental', 'normal').catch(error => {
          console.warn('自动保存后请求同步失败:', error)
        })
      } catch (error) {
        console.error('自动保存过程中出错:', error)
        errorHandler.handleError(error, '自动保存过程失败').catch(error => {
          console.warn('处理自动保存错误失败:', error)
        })
      }
    }, 2000) // 2秒延迟，避免频繁保存

    return () => clearTimeout(saveTimer)
  }, [cards, isInitialized, isLoading, errorHandler, universalStorageAdapter, requestSync])
  ...
  const saveCards = useCallback(async (cardsToSave: Card[] = cards): Promise<boolean> => {
    try {
      ...
      // 非阻塞保存：启动保存但不等待完成
      universalStorageAdapter.saveCards(validCards).then(() => {
        console.log(`成功保存 ${validCards.length} 张卡片`)
        notifyDataChange('cardsSaved', { cardCount: validCards.length })
      }).catch(error => {
        console.error('保存卡片失败:', error)
        errorHandler.handleError(error, 'saveCards失败').catch(err => {
          console.warn('处理保存错误失败:', err)
        })

        // 尝试回退保存（非阻塞）
        DataConverterAdapter.saveToLocalStorage(cardsToSave).then(() => {
          console.log('回退保存成功')
        }).catch(fallbackError => {
      ...
      console.error('保存卡片过程出错:', error)
      await errorHandler.handleError(error, 'saveCards过程失败')
      return false
    }
