...
export class SmartErrorHandler {
  ...
  async handleError(
    ...
    // 分析错误类型
    const errorAnalysis = await { log: () => {} }.categorizeAndAnalyzeError(error, context.operation)
...
export const handleSyncSmartError = (error: Error, operation: { id: string; type: string; data: any }, context?: Partial<ErrorContext>) =>
