...
export class UniversalStorageAdapter implements StorageAdapter {
  ...
  async isIndexedDBAvailable(): Promise<boolean> {
    try {
      ...
      // 尝试打开一个测试数据库来验证IndexedDB确实可用
      return new Promise((resolve) => {
        const request = indexedDB.open('cardall_test_db')
        ...
        request.onsuccess = () => {
          ...
          db.close()
          indexedDB.deleteDatabase('cardall_test_db')
          resolve(true)
        }
      })
    } catch (error) {
  ...
  async hasIndexedDBData(): Promise<boolean> {
    try {
      ...
      const [cardCount, folderCount, tagCount] = await Promise.all([
        dbInstance.cards.count().catch(() => 0),
        dbInstance.folders.count().catch(() => 0),
        dbInstance.tags.count().catch(() => 0)
      ])

      return cardCount > 0 || folderCount > 0 || tagCount > 0
    } catch (error) {
