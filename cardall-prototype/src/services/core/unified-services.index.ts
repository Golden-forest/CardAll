...
export {
  ...
  type SyncStats,
  type SyncOperation as SyncOperationLegacy,
...
export interface UnifiedServices {
  // 备份服务
  backup: {
    core: typeof backupCoreService
    scheduler: typeof backupSchedulerService
    validator: typeof backupValidatorService
    recovery: typeof backupRecoveryService
  }
  ...
  // 同步服务
  sync: typeof unifiedSyncService
}
...
export class UnifiedServicesManager {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.error('❌ Failed to initialize unified services:', error)
      throw error
    }
  }
...
export const syncService = {
  start: () => unifiedSyncService.start(),
  stop: () => unifiedSyncService.stop(),
  sync: (options?: any) => unifiedSyncService.sync(options),
  addOperation: (operation: any) => unifiedSyncService.addOperation(operation),
  getStatus: () => unifiedSyncService.getStatus(),
  getStats: () => unifiedSyncService.getStats(),
  getConflicts: () => unifiedSyncService.getConflicts(),
  resolveConflict: (conflictId: string, resolution: string, data?: any) =>
    unifiedSyncService.resolveConflict(conflictId, resolution, data),
  autoResolveConflicts: () => unifiedSyncService.autoResolveConflicts()
}
