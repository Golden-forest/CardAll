...
interface AlertProcessingContext {
  timestamp: Date
  previousMetrics: UnifiedMetrics | null
...
export class IntelligentAlertEngine {
  ...
  public async processMetrics(
    ...
    try {
      ...
      // 创建系统错误警报
      const errorAlert = this.createSystemErrorAlert(error, timestamp)
  ...
  private async detectRuleBasedAlerts(
    ...
    for (const rule of rules) {
      ...
      try {
        ...
        console.error(`评估警报规则失败 ${rule.id}:`, error)
      }
    }
    ...
    for (const alert of alerts) {
      ...
      for (const action of rule.actions) {
        try {
          ...
          console.error(`执行警报动作失败 ${alert.id}:`, error)
        }
      }
    }
  }
