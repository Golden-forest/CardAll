...
export interface CleanupStrategy {
  // 清理范围
  scope: 'full' | 'selective' | 'aggressive'
...
export class StateCleaner {
  ...
  private async validateReferenceIntegrity(): Promise<ValidationResult['issues']> {
    ...
    try {
      ...
      issues.push({
        type: 'error',
        category: 'consistency',
        message: 'Reference integrity validation failed',
        details: error,
        severity: 'critical',
        fixable: false
      })
  ...
  private async executeCleanup(strategy: CleanupStrategy, result: CleanupResult): Promise<void> {
    ...
    try {
      ...
      result.errors.push(`Cleanup execution failed: ${error}`)
      throw error
    }
  }
  ...
  private async rollbackCleanup(backupId: string): Promise<void> {
    try {
      ...
      console.error('Failed to rollback cleanup:', error)
      throw error
    }
  }
