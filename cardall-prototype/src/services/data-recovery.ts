...
export interface RecoveryPoint {
  id: string
  timestamp: number
  type: 'manual' | 'auto' | 'migration' | 'integrity_check' | 'scheduled' | 'before_update' | 'after_update' | 'emergency'
  description: string
  data: RecoveryData
  ...
  childrenPointIds?: string[]
  restoreCount: number
  lastRestoreDate?: number
...
export interface RecoveryMetadata {
  ...
  reason: string
  tags: string[]
  storageLocation: 'localStorage' | 'IndexedDB' | 'cloud'
...
export interface RecoveryOptions {
  ...
  mergeStrategy?: 'replace' | 'merge' | 'smart_merge'
  conflictResolution?: 'newer_wins' | 'older_wins' | 'manual'
...
export interface Conflict {
  id: string
  type: 'card' | 'folder' | 'tag' | 'setting'
  existingData: any
  recoveryData: any
  resolution: 'keep_existing' | 'use_recovery' | 'manual' | 'pending'
}
...
export interface RecoveryConfig {
  autoBackup: {
    ...
    interval: number // 分钟
    maxPoints: number
    triggers: ('startup' | 'shutdown' | 'data_change' | 'interval' | 'user_idle')[]
  }
  retention: {
...
**生效日期**: ${new Date().toLocaleDateString('zh-CN')}
  ...
  private async saveConfig(): Promise<void> {
    try {
      ...
      console.error('Failed to save recovery config:', error)
    }
  }
  ...
  async createBatchRecoveryPoints(types: Array<{
    ...
    for (const { type, description, metadata } of types) {
      try {
        ...
        console.error(`Failed to create recovery point ${description}:`, error)
        // 继续创建其他的恢复点
      }
    }
