...
export interface PerformanceAlert {
  ...
  category: 'performance' | 'resource' | 'network' | 'queue'
  message: string
  value: number
  threshold: number
  timestamp: Date
...
export interface TestCase {
  ...
  name: string
  description: string
  category: 'performance' | 'reliability' | 'scalability' | 'integration'
...
export interface TestResult {
  ...
  // 实际结果
  actualResults: PerformanceMetrics
  
  // 通过/失败状态
  passedChecks: TestCheck[]
  failedChecks: TestCheck[]
...
class PerformanceMonitoringService {
  ...
  private async executeTestCase(testCase: TestCase): Promise<TestResult> {
    ...
    try {
      ...
      const result: TestResult = {
        success: false,
        duration: performance.now() - testCase.startTime!.getTime(),
        actualResults: this.getCurrentMetrics(),
        passedChecks: [],
        failedChecks: [],
        errors: [{
          type: 'execution',
