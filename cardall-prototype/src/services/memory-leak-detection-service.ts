...
export interface MemoryLeakDetectionConfig {
  ...
  growthThreshold: number // 增长率阈值
  maxDetectionHistory: number
...
export interface ResourceTracker {
  id: string
  type: 'eventListener' | 'timeout' | 'interval' | 'observer' | 'subscription'
  resource: any
  created: number
  lastAccessed: number
  location: string
...
export class MemoryLeakDetectionService {
  ...
  private async applyAutomaticMitigation(report: MemoryLeakReport): Promise<void> {
    try {
      ...
      console.error('Automatic mitigation failed:', error)
    }
  }
  ...
  private cleanupAllExpiredResources(): void {
    this.resourceTrackers.forEach(tracker => {
      try {
        ...
        console.warn(`Failed to cleanup resource ${tracker.id}:`, error)
      }
    })
