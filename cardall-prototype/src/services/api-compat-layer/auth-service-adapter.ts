...
export interface AuthServiceAdapterOptions extends AdapterOptions {
  ...
  async login(email: string, password: string): Promise<AuthResult> {
    ...
    return this.wrapAsyncOperation(async () => {
      ...
      try {
        ...
        this.log('error', '用户登录异常', { email, error: errorMessage })
  ...
  async register(email: string, password: string, username?: string): Promise<AuthResult> {
    ...
    return this.wrapAsyncOperation(async () => {
      ...
      try {
        ...
        this.log('error', '用户注册异常', { email, username, error: errorMessage })
  ...
  async resetPassword(email: string): Promise<{ success: boolean; error?: string }> {
    ...
    return this.wrapAsyncOperation(async () => {
      ...
      try {
        ...
        this.log('error', '重置密码异常', { email, error: errorMessage })
  ...
  async updateProfile(updates: Partial<UserInfo>): Promise<{ success: boolean; user?: UserInfo; error?: string }> {
    ...
    return this.wrapAsyncOperation(async () => {
      ...
      try {
        ...
        this.log('error', '用户信息更新异常', { error: errorMessage })
  ...
  async changePassword(currentPassword: string, newPassword: string): Promise<{ success: boolean; error?: string }> {
    ...
    return this.wrapAsyncOperation(async () => {
      ...
      try {
        ...
        this.log('error', '密码更改异常', { error: errorMessage })
  ...
  async refreshToken(): Promise<{ success: boolean; error?: string }> {
    return this.wrapAsyncOperation(async () => {
      ...
      try {
        ...
        this.log('error', '令牌刷新异常', { error: errorMessage })
  ...
  async validateToken(): Promise<{ valid: boolean; expiresAt?: Date; error?: string }> {
    return this.wrapAsyncOperation(async () => {
      ...
      try {
        ...
        this.log('error', '令牌验证异常', { error: errorMessage })
  ...
  async getUserPermissions(): Promise<string[]> {
    return this.wrapAsyncOperation(async () => {
      ...
      try {
        ...
        this.log('warn', '获取用户权限失败，返回空权限列表', error)
        return []
      }
    }, 'getUserPermissions')
  }
  ...
  private notifyAuthStateChange(state: AuthState): void {
    this.authStateListeners.forEach(listener => {
      try {
        ...
        this.log('error', '认证状态监听器错误:', error)
      }
    })
  }
}
