...
export interface DatabaseAdapterOptions extends AdapterOptions {
  enableCache?: boolean
          ...
          await this.database.transaction('rw', [this.database.cards], async () => {
            for (const card of cardsToAdd) {
              try {
                ...
                failed++
                errors.push(`卡片 ${card.id} 添加失败: ${error instanceof Error ? error.message : '未知错误'}`)
              }
            }
          })
        } else {
          ...
          await this.database.transaction('rw', [this.database.cards], async () => {
            for (const { id, updates } of updates) {
              try {
                ...
                failed++
                errors.push(`卡片 ${id} 更新失败: ${error instanceof Error ? error.message : '未知错误'}`)
              }
            }
          })
        } else {
