...
// Note: EnhancedConflictResolution component has been removed in T015 cleanup
...
export function Dashboard({ className }: DashboardProps) {
  ...
  const handleCardDelete = async (cardId: string) => {
    try {
      await cardDispatch({ type: 'DELETE_CARD', payload: cardId })
      toast({
        title: "卡片已删除",
        description: "卡片已成功删除。",
      })
  ...
  const handleCardMoveToFolder = async (cardId: string, folderId: string | null) => {
    try {
      const card = cards.find((c: any) => c.id === cardId)
      if (!card) return
  ...
  const handleCreateCard = async () => {
    try {
      ...
      await cardDispatch({ type: 'CREATE_CARD', payload: newCard })
      toast({
        title: "卡片已创建",
        description: "新卡片已成功创建。",
      })
    } catch (error) {
      console.error('创建卡片失败:', error)
      toast({
        title: "创建失败",
        description: "无法创建卡片，请稍后再试。",
        variant: "destructive",
      })
    }
  }
  ...
  const handleConfirmCreateFolder = async (folderData: {
    name: string
    color: string
    icon: string
    parentId?: string
    ...
    if (deletingFolder) {
      try {
        await folderDispatch({
          ...
          onDeleteCards: async (cardIds: string[]) => {
  ...
  return (
    <FolderPanelProvider>
      <div className={cn("min-h-screen bg-background", className)}>
        ...
        {/* Conflict Management Modals - Component removed in T015 cleanup */}
        {/*
        <EnhancedConflictResolution
          conflicts={conflicts}
          onResolve={async (conflictId, resolution, reason) => {
            // Handle conflict resolution
          }}
          onIgnore={async (conflictId) => {
            // Handle conflict ignore
          }}
          onBatchResolve={async (conflictIds, resolution, reason) => {
            // Handle batch resolution
          }}
          onAutoResolve={async (conflictIds) => {
