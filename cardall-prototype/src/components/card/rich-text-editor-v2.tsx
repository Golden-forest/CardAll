...
export function RichTextEditorV2({
  ...
  const handleImageUpload = useCallback(async (files: File[]) => {
    ...
    try {
      ...
      // 批量处理图片
      const results = await imageProcessorService.processImages(imageFiles, cardId, folderId)
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        // 获取图片URL
        const imageUrl = await imageProcessorService.getImageUrl(result.filePaths.webp)
