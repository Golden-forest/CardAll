...
export interface StorageEvent {
  id: string
  type: 'read' | 'write' | 'delete' | 'sync' | 'error' | 'warning'
  operation: string
  timestamp: Date
  duration: number
  dataSize?: number
...
export interface StorageIssue {
  id: string
  type: 'performance' | 'reliability' | 'availability' | 'efficiency' | 'security'
...
export interface DiagnosticReport {
  ...
  // 概要信息
  summary: {
    overallHealth: StorageHealthStatus['overall']
    totalIssues: number
...
export interface DiagnosticRecommendation {
  ...
  category: 'performance' | 'reliability' | 'availability' | 'efficiency'
  title: string
  description: string
  impact: string
...
export interface StorageMonitoringConfig {
  ...
  alertThresholds: StorageAlertThresholds
