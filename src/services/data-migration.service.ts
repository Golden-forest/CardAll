...
export class DataMigrationService {
  ...
  private async performMigration(
    ...
    try {
      ...
      console.warn('Bulk migration failed, trying individual migration:', bulkError)
      ...
      for (let i = 0; i < cards.length; i++) {
        ...
        try {
          const dbCard = DataConverterAdapter.cardToDbCard(card)
          await db.cards.add(dbCard)
          migratedCount++

          if (progressCallback) {
            const progress = (i + 1) / totalCards
            progressCallback(progress, `Migrated card ${i + 1}/${totalCards}: ${card.frontContent.title}`)
          }
        } catch (cardError) {
      ...
      if (!success) {
        warnings.push(`Some cards failed to migrate: ${errors.length} out of ${totalCards}`)
      }
...
export interface MigrationProgress {
  stage: 'starting' | 'loading' | 'backup' | 'validation' | 'migration' | 'verification' | 'cleanup' | 'completed' | 'error'
