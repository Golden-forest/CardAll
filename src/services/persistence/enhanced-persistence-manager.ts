import { db, type DbCard, type DbFolder, type DbTag, type DbImage, type SyncOperation, type AppSettings, type UserSession } from '@/services/database'
...
export interface ConsistencyCheckResult {
  id: string
  checkType: 'entity-count' | 'reference-integrity' | 'data-integrity' | 'sync-consistency'
  entityType?: 'card' | 'folder' | 'tag' | 'image' | 'sync'
...
export interface StateRecoveryOptions {
  // 恢复范围
  scope: 'full' | 'app-state' | 'sync-state' | 'user-session' | 'performance-state'
...
export class EnhancedPersistenceManager {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.error('Failed to initialize EnhancedPersistenceManager:', error)
      throw error
    }
  }
  ...
  async persistState(state: Partial<PersistenceState>): Promise<void> {
    ...
    try {
      ...
      await this.notifyError(error as Error, 'persistState')
      throw error
    } finally {
  ...
  async restoreState(options: StateRecoveryOptions = {
    ...
    try {
      ...
      if (options.rollbackOnError) {
        await this.rollbackStateRestoration()
      }
  ...
  async clearPersistedState(scope: 'full' | 'app-state' | 'sync-state' | 'user-session' | 'performance-state' = 'full'): Promise<void> {
    ...
    try {
      ...
      await this.notifyError(error as Error, 'clearPersistedState')
      throw error
    } finally {
      this.currentOperation = null
    }
  ...
  async performFullConsistencyCheck(): Promise<ConsistencyCheckResult[]> {
    ...
    try {
      ...
      console.error('Failed to perform consistency check:', error)
      await this.notifyError(error as Error, 'performFullConsistencyCheck')
      throw error
    } finally {
      this.currentOperation = null
    }
  ...
  async performQuickConsistencyCheck(): Promise<ConsistencyCheckResult[]> {
    ...
    try {
      ...
      await this.notifyError(error as Error, 'performQuickConsistencyCheck')
      throw error
    } finally {
      this.currentOperation = null
    }
  ...
  async fixConsistencyIssues(issues: ConsistencyCheckResult[]): Promise<number> {
    ...
    try {
      ...
      await this.notifyError(error as Error, 'fixConsistencyIssues')
      throw error
    } finally {
      this.currentOperation = null
    }
  ...
  async createBackup(options: BackupOptions): Promise<string> {
    ...
    try {
      ...
      await this.notifyError(error as Error, 'createBackup')
      throw error
    } finally {
      this.currentOperation = null
    }
  ...
  async restoreFromBackup(backupId: string, options: StateRecoveryOptions): Promise<void> {
    ...
    try {
      ...
      await this.notifyError(error as Error, 'restoreFromBackup')
      throw error
    } finally {
      this.currentOperation = null
    }
  ...
  async optimizePerformance(options: PerformanceOptimizationOptions): Promise<{
    ...
    try {
      ...
      await this.notifyError(error as Error, 'optimizePerformance')
      throw error
    } finally {
      this.currentOperation = null
    }
  ...
  private async verifyStateRestoration(state: PersistenceState): Promise<void> {
    ...
    try {
      ...
      throw new Error('Database connection failed after state restoration')
    }
  }
  ...
  private async checkEntityCounts(): Promise<ConsistencyCheckResult[]> {
    ...
    try {
      ...
      results.push({
        id: `entity-count-error-${Date.now()}`,
        checkType: 'entity-count',
        status: 'failed',
        severity: 'high',
  ...
  private async checkReferenceIntegrity(): Promise<ConsistencyCheckResult[]> {
    ...
    try {
      ...
      results.push({
        id: `reference-integrity-error-${Date.now()}`,
        checkType: 'reference-integrity',
        status: 'failed',
        severity: 'high',
  ...
  private async checkDataIntegrity(): Promise<ConsistencyCheckResult[]> {
    ...
    try {
      ...
      results.push({
        id: `data-integrity-error-${Date.now()}`,
        checkType: 'data-integrity',
        status: 'failed',
        severity: 'high',
  ...
  private async checkSyncConsistency(): Promise<ConsistencyCheckResult[]> {
    ...
    try {
      ...
      results.push({
        id: `sync-consistency-error-${Date.now()}`,
        checkType: 'sync-consistency',
        status: 'failed',
        severity: 'high',
  ...
  private async fixConsistencyIssue(issue: ConsistencyCheckResult): Promise<boolean> {
    try {
      ...
      console.error(`Failed to fix consistency issue ${issue.id}:`, error)
      return false
    }
  }
  ...
  private async rebuildIndexes(): Promise<{ success: boolean; rebuiltIndexes: string[] }> {
    try {
      ...
      return { success: false, rebuiltIndexes: [] }
    }
  }
  ...
  private async cleanupOrphanedData(): Promise<{ success: boolean; cleanedItems: number }> {
    try {
      ...
      return { success: false, cleanedItems: 0 }
    }
  }
  ...
  private async optimizeCacheSize(): Promise<{ success: boolean; optimizedSize: number }> {
    try {
      ...
      return { success: false, optimizedSize: 0 }
    }
  }
  ...
  private async updateStatistics(): Promise<{ success: boolean; updatedStats: any }> {
    try {
      ...
      return { success: false, updatedStats: null }
    }
  }
  ...
  private async defragmentDatabase(): Promise<{ success: boolean; spaceSaved: number }> {
    try {
      ...
      console.error('Failed to defragment database:', error)
  ...
  private async notifyStateSaved(state: PersistenceState): Promise<void> {
    if (this.listeners.stateRestored) {
      try {
        ...
        console.error('Error in stateSaved listener:', error)
      }
    }
  }
  ...
  private async notifyStateRestored(state: PersistenceState): Promise<void> {
    if (this.listeners.stateRestored) {
      try {
        ...
        console.error('Error in stateRestored listener:', error)
      }
    }
  }
  ...
  private async notifyConsistencyChecked(results: ConsistencyCheckResult[]): Promise<void> {
    if (this.listeners.consistencyChecked) {
      try {
        ...
        console.error('Error in consistencyChecked listener:', error)
      }
    }
  }
  ...
  private async notifyBackupCreated(backupId: string): Promise<void> {
    if (this.listeners.backupCreated) {
      try {
        ...
        console.error('Error in backupCreated listener:', error)
      }
    }
  }
  ...
  private async notifyOptimizationCompleted(results: any): Promise<void> {
    if (this.listeners.optimizationCompleted) {
      try {
        ...
        console.error('Error in optimizationCompleted listener:', error)
      }
    }
  }
  ...
  private async notifyError(error: Error, context: string): Promise<void> {
    if (this.listeners.errorOccurred) {
      try {
        ...
        console.error('Error in errorOccurred listener:', listenerError)
      }
    }
  ...
  async destroy(): Promise<void> {
    try {
      ...
      console.error('Failed to destroy EnhancedPersistenceManager:', error)
    }
  }
}
...
export const initializeEnhancedPersistence = async (): Promise<void> => {
  try {
    ...
    console.error('Failed to initialize enhanced persistence system:', error)
    throw error
  }
}