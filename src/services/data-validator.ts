...
export interface ValidationResult {
  isValid: boolean
  score: number // 0-100 数据质量分数
  errors: ValidationError[]
  warnings: ValidationWarning[]
  stats: DataValidationStats
  recommendations: string[]
}

export interface ValidationError {
  id: string
  type: 'missing_field' | 'invalid_format' | 'reference_broken' | 'constraint_violation' | 'data_corruption'
...
export interface ValidationWarning {
  id: string
  type: 'performance' | 'consistency' | 'best_practice' | 'deprecation'
  severity: 'low' | 'medium' | 'high'
  entityType: 'card' | 'folder' | 'tag' | 'image' | 'setting' | 'system'
  entityId?: string
  field?: string
  message: string
...
class DataValidatorService {
  ...
  private async validateDataConsistency(): Promise<{ errors: ValidationError[]; warnings: ValidationWarning[] }> {
    ...
    try {
      ...
      errors.push({
        id: 'consistency-validation-error',
        type: 'data_corruption',
        severity: 'critical',
        entityType: 'system',
        entityId: 'system',
        message: `一致性验证失败: ${error}`,
