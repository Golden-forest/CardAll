...
export function Dashboard({ className }: DashboardProps) {
  ...
  const renderFolderTree = (folders: any[] = [], level = 0) => {
    console.log('🌳 renderFolderTree called with folders:', folders.length, 'at level:', level)
    ...
    try {
      return folders.map(folder => {
        ...
        console.log('🌳 Rendering folder:', folder.name, 'with cardIds:', folder.cardIds)
  ...
  const renderCollapsedFolderTree = (folders: any[] = []) => {
    ...
    return folders.map(folder => {
      // 安全检查
      if (!folder || !folder.id) {
        console.warn('🌳 Invalid folder data in collapsed view:', folder)
        return null
      }
                  ...
                  <div className="space-y-1">
                    ...
                    {process.env.NODE_ENV === 'development' && (
                      <div className="text-xs text-gray-500 mb-2">
                        ...
                        <div>树形结构数量: {folderTree.length}</div>
